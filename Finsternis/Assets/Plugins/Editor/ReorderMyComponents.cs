...
public class ReorderMyComponents : EditorWindow {
	...
	private static void ShowWindow () {
		ReorderMyComponents windowHndle = (ReorderMyComponents)GetWindow(typeof(ReorderMyComponents));
    ...
    void OnGUI () {
		...
		if (selectedObject != null) {
			...
			if (mouseDown && Event.current.type == EventType.MouseUp) {
				...
				if (positionsToMove > 0) {
					...
					for (int i=0; i<positionsToMove; i++) {
                        ...
                        comps = selectedObject.GetComponents<Component>();
					}
				}
            }
            ...
            for (int i=0; i<comps.Length; i++) {
                ...
                int index = comps[j].GetType().ToString().LastIndexOf('.') + 1;

                string componentName = comps[j].GetType().ToString().Substring(index);
